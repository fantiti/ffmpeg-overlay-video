<?php
	if(is_uploaded_file($_FILES['uploaded_file']['tmp_name'])) {
		...
		$targetPath = "temp/".$_FILES['uploaded_file']['name'];
			if(move_uploaded_file($sourcePath,$targetPath)) { 
				?>
				<div>
    		        <video id="vid" autostart="false" loop="" style="margin-top: -8px; margin-left: -8px; width: 400px;">
    ...
    if(is_uploaded_file($_FILES['uploaded_file1']['tmp_name'])) {
        $sourcePath1 = $_FILES['uploaded_file1']['tmp_name'];
        $targetPath1 = "temp/".$_FILES['uploaded_file1']['name'];
            if(move_uploaded_file($sourcePath1,$targetPath1)) { 
                ?>
                <div>
                    <img src="<?php echo $targetPath1; ?>" style="width: 50%;">
                </div>
                <?php
            }
    }
...
<body>
    ...
    <div class="row">
    <form method="post" action="overlay.php" enctype="multipart/form-data">
        <div class="input-group">
        <input type="hidden" name="videofile" id="videofile" value="<?php echo $targetPath; ?>">
        <input type="hidden" name="imagefile" id="imagefile" value="<?php echo $targetPath1; ?>">
           
            <span class="input-group-btn">
                <input class="btn btn-primary" type="submit" name="overlay" id="overlay" value="Generate Overlay">
            </span>
            <span> Tạo clip</span>
        </div>
    </form>
